...
def shift_by_params(periods, time_frame, col_name=None):
    ...
    """
    min_valid_index = periods * (time_frame // 10)
    df_truncated = pl.col("index") >= min_valid_index

    idx = df_truncated - (...
    )

    validated_idx = pl.when(idx > 10)
...
def create_shifted_col(
    df, pair_name, periods, time_frame, columns=["OPEN", "CLOSE"]
):  
...
def drop_first_day_polars(df):
    ...
    first_date = df.select(pl.col("_time")).row(0)[0]

    # Filter the DataFrame to drop rows with the first date
    df_filtered = df.filter(pl.col("_time") != first_date)
